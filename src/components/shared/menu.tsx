...
export const Menu = () => {
  ...
  return (
    <nav className={`animate__animated animate__fadeInDown bg-[#000] fixed flex w-full items-center justify-between px-[10vw] py-[1.2rem] ${(y ?? 0) > 100 ? 'bg-black bg-opacity-30 backdrop-blur-[5px]' : 'bg-transparent'} z-50`} aria-label="Main navigation">
      ...
      {isMenuOpen && (
        <div
          className="fixed top-0 left-0 h-full w-full bg-black text-white p-6 flex flex-col space-y-4 mt-16 transition-transform transform duration-300 ease-in-out md:hidden"
        >
          {menuItems.map((item) => (
            <Link key={item.href} href={item.href} className="hover:text-gray-400 text-lg" onClick={toggleMenu}>
              {item.label}
            </Link>
          ))}
        </div>
